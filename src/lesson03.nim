...
var meshes : seq[TMesh]
proc processNode(node: PNode, scene: ptr TScene) =
  #echo scene.meshCount
...
proc loadModel(path: string) = 
  var scn = aiImportFile(path, aiProcess_Triangulate or aiProcess_FlipUVs or aiProcess_CalcTangentSpace)
  if scn == nil  or scn.rootNode == nil: #or scn.flags == 1
    echo "Error::AssImp::", getError()
    quit(-1)
  #var directory = path.find('/')
  processNode( scn.rootNode, scn )
...
#[
  ...
  var 
    ...
    vertexShaderSource = loadShaderFromFile("shaders/mvp-vert.glsl")
  ...
  var 
    ...
    fragmentShaderSource = loadShaderFromFile("shaders/mvp-frag.glsl")
...
#[
  ...
  while not windowShouldClose(window):
    ...
    var
      ...
      camTgt = vec3(0.0f, 0.0f, 0.0f)
      camDir = normalize(camPos - camTgt)
      camRgt = normalize(cross(up, camDir))
      camUpx = cross(camDir, camRgt)
    ...
    glUniformMatrix4fv(modelLoc, 1.GLsizei, GL_FALSE.GLboolean, addr model[0][0])
    glUniformMatrix4fv(viewLoc, 1.GLsizei, GL_FALSE.GLboolean, addr view[0][0])

    glUniformMatrix4fv(glGetUniformLocation(shaderProgram, "projection"), 1.GLsizei, GL_FALSE.GLboolean, addr proj[0][0] )
    glBindVertexArray(vao[0])
